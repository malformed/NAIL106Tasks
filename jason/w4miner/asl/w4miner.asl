...
{ include("w4allocation_protocol.asl") }  // plans for the gold allocation protocol
...
+!inform_gsize_to_leader(S) : .my_name(w4miner1) 
   <- ?depot(S,DX,DY);
      .send(w4leader,tell,depot(S,DX,DY));
      ?gsize(S,W,H); 
      .send(w4leader,tell,gsize(S,W,H)).
