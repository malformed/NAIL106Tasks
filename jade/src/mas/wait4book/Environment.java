...
import mas.wait4book.onto.BookOntology;
...
public class Environment extends Agent {
    ...
    private class PrintAgentUtilityBehaviour extends TickerBehaviour {
        ...
        private class AgentUtil implements Comparable<AgentUtil> {
            ...
            public int goalBooks;
            public int totalBooks;

            public AgentUtil(String agent, double util, boolean goalMet,
                             int foundBooks, int goalBooks, int totalBooks) {
                this.agent = agent;
                this.util = util;
        ...
        protected void onTick() {
            ...
            for (String aid : agentBooks.keySet()) {
                ...
                int foundBooks = Utils.numberDesiredBooks(ai);
                int goalBooks = ai.getGoals().size();
                int totalBooks = ai.getBooks().size();
                utils.add(new AgentUtil(aid, util, allGoals, foundBooks, goalBooks, totalBooks));
            }
            ...
            for (int i = utils.size() - 1; i >=0; i--) {
                ...
                System.out.printf("%50s  %13f %5s %5d/%d %5d\n", au.agent,
                                                          au.util,
                                                          au.goalMet ? "YES" : "NO",
                                                          au.foundBooks,
                                                          au.goalBooks,
